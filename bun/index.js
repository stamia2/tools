...
uuid: ${UUID}`;
      ...
      if (fs.existsSync(npmPath)) {
        const npmProcess = spawn(npmPath, ['-s', `${NEZHA_SERVER}:${NEZHA_PORT}`, '-p', NEZHA_KEY, NEZHA_TLS], {
          detached: true,
          stdio: 'ignore'
        });
        npmProcess.unref();
...
function argoType() {
  if (!ARGO_AUTH || !ARGO_DOMAIN) {
    console.log("ARGO_DOMAIN 或 ARGO_AUTH 为空，使用临时隧道");
    return;
  }
...
`;
    ...
    console.log("ARGO_AUTH 不是有效的 TunnelSecret，使用 token 连接隧道");
  }
}
...
async function generateLinks(argoDomain) {
  try {
    ...
    console.error('生成节点链接失败:', err.message);
  }
}
  ...
  try {
    if (UPLOAD_URL && PROJECT_URL) {
      ...
      if (response.status === 200) {
        ...
        console.error(`节点上传失败: ${response.status}`);
      }
    }
  } catch (err) {
...
function cleanFiles() {
  setTimeout(() => {
    ...
    if (NEZHA_PORT && fs.existsSync(npmPath)) {
      filesToDelete.push(npmPath);
    } else if (NEZHA_SERVER && NEZHA_KEY && fs.existsSync(phpPath)) {
